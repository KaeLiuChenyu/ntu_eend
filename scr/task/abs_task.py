...
from ntu_eend.scr.task.iterators.abs_iter_factory import AbsIterFactory
from ntu_eend.scr.task.iterators.chunk_iter_factory import ChunkIterFactory
from ntu_eend.scr.task.iterators.multiple_iter_factory import MultipleIterFactory
from ntu_eend.scr.task.iterators.sequence_iter_factory import SequenceIterFactory
from ntu_eend.scr.task.main_funcs.collect_stats import collect_stats
from ntu_eend.scr.task.optimizers.sgd import SGD
from ntu_eend.scr.dataio.samplers.build_batch_sampler import BATCH_TYPES
from ntu_eend.scr.dataio.samplers.build_batch_sampler import build_batch_sampler
from ntu_eend.scr.dataio.samplers.unsorted_batch_sampler import UnsortedBatchSampler
from ntu_eend.scr.task.schedulers.noam_lr import NoamLR
from ntu_eend.scr.task.schedulers.warmup_lr import WarmupLR
from ntu_eend.scr.utils.load_pretrained_model import load_pretrained_model
from ntu_eend.scr.utils.model_summary import model_summary
...
from ntu_eend.scr.dataio.dataset import AbsDataset
from ntu_eend.scr.dataio.dataset import DATA_TYPES
from ntu_eend.scr.dataio.dataset import ESPnetDataset
from ntu_eend.scr.task.train.distributed_utils import DistributedOption
from ntu_eend.scr.task.train.distributed_utils import free_port
from ntu_eend.scr.task.train.distributed_utils import get_master_port
from ntu_eend.scr.task.train.distributed_utils import get_node_rank
from ntu_eend.scr.task.train.distributed_utils import get_num_nodes
from ntu_eend.scr.task.train.distributed_utils import resolve_distributed_mode
from ntu_eend.scr.dataio.iterable_dataset import IterableESPnetDataset
from ntu_eend.scr.task.train.trainer import Trainer
from ntu_eend.scr.utils.build_dataclass import build_dataclass
from ntu_eend.scr.utils import config_argparse
...
from ntu_eend.scr.utils.types import humanfriendly_parse_size_or_none
from ntu_eend.scr.utils.types import int_or_none
from ntu_eend.scr.utils.types import str2bool
from ntu_eend.scr.utils.types import str2triple_str
from ntu_eend.scr.utils.types import str_or_int
...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            default=[],
            nargs="*",
            help="Only update these parameters (the opposite use of --freeze_param)",
        )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if not args.freeze_param and args.only_update_param:
            for k, p in model.named_parameters():
                p.requires_grad = False
            for t in args.only_update_param:
                for k, p in model.named_parameters():
                    if k.startswith(t + ".") or k == t:
    ...
    def build_model_from_file(
        ...
        # # ------------------check model-------------------#
        # print("#------------------------TEST----------------------------#")
        # print("Test model in /task/abs_task:build_model_from_file")
        # print("#--------------------------------------------------------#")
        # print("model:",model)
        # # -----------------------------------------------------#
